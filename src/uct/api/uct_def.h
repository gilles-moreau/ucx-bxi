...
enum uct_cb_param_flags {
    ...
    UCT_CB_PARAM_FLAG_HW_RNDV = UCS_BIT(3)
};
...
typedef struct uct_gop               *uct_gop_h; /**< @brief Generic Operation handle */
...
/**
 * @ingroup UCT_TAG
 * @brief Callback to process expected rendezvous tagged message.
 *
 * This callback is invoked when rendezvous send notification has arrived
 ...
 *
 */
typedef void (*uct_tag_exp_rndv_cb_t)(void *arg,
                                      uint64_t stag, const void *header,
                                      unsigned header_length,
                                      uint64_t remote_addr, size_t length,
                                      const void *rkey_buf, 
