                              ...
                              void *arg, unsigned flags)
{
    ...
    void         *start = NULL;
    ssize_t       size  = 0;
    ecr_ptl_op_t *op    = ecc_mpool_pop(ptl_ep->am_mp);
    ...
    rc = ecr_ptl_wrap(PtlPut(ptl_ep->am_md->mdh, (ptl_size_t)start, size,
                             PTL_CT_ACK_REQ, ptl_ep->super.pid, ptl_ep->am_pti,
                             0, 0, NULL, ECR_PTL_HDR_SET(hdr, id)));
    ...
    ecc_queue_push(&ptl_ep->am_md->opq, &op->elem);
                                  ...
                                  ecr_completion_t *comp)
{
    ...
    rc = ecr_ptl_wrap(PtlGet(ptl_ep->rma_md->mdh, (ptl_size_t)local_addr, size,
                             ptl_ep->super.pid, ptl_ep->rma_pti, 0, remote_addr,
                             NULL));
...
ECC_CLASS_DEFINE_INIT_FUNC(ecr_ptl_am_ep_t, ecr_iface_h iface,
                           ecr_iface_addr_t *addr, unsigned flags)
{
    ...
    self->am_mp  = &ptl_iface->am_mp;
    self->rma_mp = &ptl_iface->rma_mp;
    self->am_md  = &ptl_iface->am_md;
    self->rma_md = ptl_iface->rma_md;

    self->am_pti  = ptl_addr->am_pti;
    self->rma_pti = ptl_addr->rma_pti;

    return ECC_SUCCESS;
}

ECC_CLASS_DEFINE_CLEAN_FUNC(ecr_ptl_am_ep_t)
{
    ECC_CLASS_CALL_SUPER_CLEAN(ecr_ptl_ep_t, self);
    return;
}

ECC_CLASS_DEFINE(ecr_ptl_am_ep_t, ecr_ptl_ep_t);
                                  ...
                                  unsigned flags, ecr_ep_h *ep_p)
{
    ...
    rc = ECC_CLASS_NEW(ecr_ptl_am_ep_t, &ep, iface, addr, flags);
    if (rc != ECC_SUCCESS)
        goto err;
...
{
    ECC_CLASS_DELETE(ecr_ptl_am_ep_t, ep);
