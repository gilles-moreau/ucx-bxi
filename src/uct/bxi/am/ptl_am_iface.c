...
static ucs_mpool_ops_t uct_ptl_send_op_mpool_ops = {
...
static UCS_CLASS_INIT_FUNC(uct_ptl_am_iface_t, uct_md_h tl_md,
  ...
  mp_ops_param = (ucs_mpool_params_t){
      ...
      .alignment = 64,
      .align_offset = 0,
      .ops = &uct_ptl_send_op_mpool_ops,
      .name = "rma-am-ops",
      .grow_factor = 1,
  };
  rc = ucs_mpool_init(&mp_ops_param, &self->rma_mp);
  if (rc != UCS_OK)
    goto err;
  ...
  rc = ucs_mpool_init(&mp_ops_param, &self->am_mp);
  if (rc != UCS_OK)
    goto err;
